...
Explică fiecare parte a adresei: `https://ursoaia-edu.online/guide/python/04-conditii/`

??? success "Soluție"
    - `https` — **protocolul**: cum comunicăm (HTTPS = HTTP securizat).
    - `ursoaia-edu.online` — **domeniul**: adresa serverului.
    - `/guide/python/04-conditii/` — **path-ul**: ce pagină anume de pe acel server vrem să vedem.
